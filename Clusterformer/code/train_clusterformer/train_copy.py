...
from train_parameters_copy import n_class,in_wh,model_name
from train_parameters_copy import train_map,train_label,val_map,val_label
from train_parameters_copy import map_seffix,label_seffix
from train_parameters_copy import augmentation_methods

from train_parameters_copy import project_name,model_name,in_chans,out_chans
from train_parameters_copy import batch_size,epoch_max,epoch_from,num_workers
from train_parameters_copy import lr_start,lr_decay,opt_name,loss_name
from train_parameters_copy import gpu
from train_parameters_copy import model_dir,tensorboard_log_dir
from train_parameters_copy import continue_checkpoint_optim_file_name,continue_checkpoint_model_file_name
   ...
   # else:
    ...
    for it, (images, labels, names) in enumerate(train_loader):
        ...
        with autocast():
            ...
            logits,y0,y1,y2,y3 = model(images)
            logits,y0,y1,y2,y3 = logits.squeeze(1),y0.squeeze(1),y1.squeeze(1),y2.squeeze(1),y3.squeeze(1)
            loss = eval(loss_name)(logits=logits,labels=labels.float())
            loss = loss.loss_function()
            
            labels0 = F.avg_pool2d(labels.float(),4,4)>0
            loss0 = eval(loss_name)(logits=y0,labels=labels0.float())
            loss0 = loss0.loss_function()

            labels1 = F.avg_pool2d(labels0.float(),2,2)>0
            loss1 = eval(loss_name)(logits=y1,labels=labels1.float())
            loss1 = loss1.loss_function()

            labels2 = F.avg_pool2d(labels1.float(),2,2)>0
            loss2 = eval(loss_name)(logits=y2,labels=labels2.float())
            loss2 = loss2.loss_function()

            labels3= F.avg_pool2d(labels2.float(),2,2)>0
            loss3 = eval(loss_name)(logits=y3,labels=labels3.float())
            loss3 = loss3.loss_function()

            # labels_deep= F.adaptive_avg_pool2d(labels3.float(),1).squeeze(-1).squeeze(-1)>0
            # loss4 = eval(loss_name)(logits=y_clf.squeeze(-1),labels=labels_deep.float())
            # loss4 = loss4.loss_function()            
            iouloss = SoftIoULoss(1)
            iouloss = iouloss(logits,labels)
            loss =  loss  +  loss2 +  loss3 + loss0 + loss1#+ 
...
def main():
    ...
    model = eval(model_name_all)(in_chans=in_chans,n_class=out_chans)
    ...
    if epoch_from > 1:
        ...
        model.load_state_dict(torch.load(continue_checkpoint_model_file_name, map_location='cuda:1'))
        # optimizer.load_state_dict(torch.load(continue_checkpoint_optim_file_name))
        print('done!')
